...
impl UpdaterUI {
    ...
    pub unsafe fn new_with_precheck(app_ui: &Rc<AppUI>) -> Result<()> {
        ...
        if update_available {
            Self::new(app_ui, updates_for_program, updates_for_schema)?;
        }

        Ok(())
    }

    pub unsafe fn new(app_ui: &Rc<AppUI>, precheck_program: Option<APIResponse>, precheck_schema: Option<GitResponse>) -> Result<()> {
...
pub fn check_updates_main_program() -> Result<APIResponse> {
    let update_channel = update_channel();
    let last_release = last_release(update_channel)?;
